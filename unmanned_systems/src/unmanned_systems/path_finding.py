...
def astar(start_position, goal_position, configSpace, iter_max, obstacle_radius):
    """start position, goal position, config space
    legal moves, obstacle_list"""
    turtle = Turtle(start_position[0],start_position[1],configSpace.spacing)
    ...
    while [current_node.x, current_node.y] != goal_position:
        ...
        for move in turtle.move_list:
            ...
            heuristic = compute_distance(new_position, goal_position)
            cost = current_node.cost + compute_distance(new_position, turtle.position) + heuristic
            ...
            if configSpace.check_within_obstacle(
                    configSpace.obstacles, new_position, obstacle_radius) == True:
                continue 
